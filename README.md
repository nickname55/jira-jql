...
Чтобы расширить нашу JQL функцию,

вы можете заставить ее принимать параметр.

Кроме того, фукнциям обычно необходимо

проверять разрешения в Jira,

чтобы фукнция возвращала только те проекты,

к которым у пользователя есть доступ.

В нашем случае, поскольку мы используем функцию недавней истории,

мы уже знаем, что пользователь имеет доступ

к возвращаемому функцией списку задач.
...
Для получения большей информации о JQL функциях, познакомьтесь
...
Advanced topics
https://developer.atlassian.com/server/framework/atlassian-sdk/advanced-topics/
